...
| **P02** | 設定画面 | `settings` |
| **P03** | メイン画面 | `main` |
| **P10** | 入荷 - 倉庫選択画面 | `incoming_warehouse_selection` |
| **P11** | 入荷 - 商品リスト画面 | `incoming_product_list` |
| **P12** | 入荷 - スケジュールリスト画面 | `incoming_schedule_list` |
| **P13** | 入荷 - 入荷入力画面 | `incoming_input` |
| **P14** | 入荷 - 入荷履歴画面 | `incoming_history` |
| **P20** | 出荷 - コース選択画面 | `picking_list` |
| **P21** | 出荷 - データ入力画面 | `outbound_picking/{taskId}` |
| **P22** | 出荷 - 出荷履歴画面 | `picking_history/{taskId}` |
| **P30** | 移動処理（未実装） | `move` |
...
| **UI要素** | 入荷処理ボタン、出荷処理ボタン、移動処理ボタン、棚卸処理ボタン、ロケ検索ボタン |
| **フッター** | 日付、ホストURL、ログアウトボタン、バージョン |
| **遷移先** | P10 倉庫選択、P20 コース選択、P02 設定、P01 ログイン（ログアウト時） |

---

## P10 入荷 - 倉庫選択画面 (WarehouseSelectionScreen)
...
## P11 入荷 - 商品リスト画面 (ProductListScreen)
...
| **遷移先** | P12 スケジュールリスト、P14 入荷履歴 |
| **備考** | 広い画面を活かし、1行に容量・温度帯・数量バッジを表示。バーコードスキャン→検索対応。300msデバウンス |

---

## P12 入荷 - スケジュールリスト画面 (ScheduleListScreen)
...
| **UI要素** | 商品情報ヘッダー、入荷日表示、入荷数量入力（予定数表示付き）、賞味期限入力（カレンダーピッカー付き）、ロケーション入力（オートコンプリート） |
...
| **遷移先** | P21 データ入力、P22 出荷履歴 |

---

## P21 出荷 - データ入力画面 (OutboundPickingScreen)
...
| **ファンクションキー** | F2: 履歴、F4: 戻る |
| **遷移先** | P22 出荷履歴、P20 コース選択 |

---

## P22 出荷 - 出荷履歴画面 (PickingHistoryScreen)
