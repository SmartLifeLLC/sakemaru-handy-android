...
P21（出庫データ入力画面）とP22（出庫履歴画面）のボディレイアウトを変更する。
P21の右側履歴を削除し、受注数/出庫数を右側へ移動。P22のボディにP21から削除した履歴リストを配置。
...
### 現在のP21レイアウト構造
- 2ペイン構成（左50%: 商品情報+受注数/出庫数+ボタン、右50%: 履歴リスト）
- 左ペイン: 商品名/JAN/容量、ロケーション/伝票番号、受注数(読取専用)/出庫数(入力)、前へ/登録/次へボタン
- 右ペイン: 履歴リスト（HistoryItemRow）

### 現在のP22レイアウト構造
- 読取専用モード: 完了アイコン+メッセージ
- 編集モード: 2列グリッド（LazyVerticalGrid）でカード表示
- 各カード: 商品名/伝票番号/規格/JAN/予定数量/出庫数量
...
- 実績:
  ...
  - 右ペイン: 受注数/出庫数セクションを削除し、ケース（受注数：XX）/バラ（受注数：XX）2列レイアウトに変更
  - 入力欄を右ペイン最上部に配置
...
- 実績:
  ...
  - 各カード: 商品名+削除ボタン / JAN・伝票番号 / 出庫数・受注数 の3行構成
