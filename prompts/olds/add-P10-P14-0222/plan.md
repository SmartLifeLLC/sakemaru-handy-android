...
コミット `8864103 入荷処理機能（P10〜P14）の実装` により、以下がすでに実装済み:
...
| 2 | GET | `/api/incoming/schedules` | `warehouse_id=<ID>` | 入荷予定商品リスト |
...
2. `MainScreen.kt` の「入荷処理」ボタンが `incoming_warehouse_selection` に遷移するか確認
...
```
P03 メイン → P10 倉庫選択 → P11 商品リスト → P12 スケジュールリスト → P13 入荷入力 → P12
                                    ↓
                              P14 入荷履歴 ←→ P13 入荷入力（編集）
```
...
**P11 商品リスト**
- [ ] 入荷予定商品リストが API から正常ロードされる（`GET /api/incoming/schedules`）
...
**P14 入荷履歴**
- [ ] 本日の入荷履歴が API から正常ロードされる（`GET /api/incoming/work-items?status=all&from_date=today`）
